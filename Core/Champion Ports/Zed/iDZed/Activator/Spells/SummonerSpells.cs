    ...
    static class SummonerSpells
    {
        ...
        public static bool Cast(this SummonerSpell spell, AIHeroClient tgHero)
        {
            return IsReady(spell) &&
                (DistanceEx.Distance(ObjectManager.Player, tgHero) < spell.Range * spell.Range) &&
        ...
        {
            return IsReady(spell) &&
                (ObjectManager.Player.Distance(position) < spell.Range * spell.Range) &&
                ObjectManager.Player.Spellbook.CastSpell(spell.Slot, Vector2Extensions.ToVector3(position));
        }
