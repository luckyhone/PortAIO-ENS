    ...
    class Jinx : Base
    {
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            if (qRange.Enabled)
            {
                if (FishBoneActive)
                {
                    CircleRender.Draw(Player.Position, 590f + Player.BoundingRadius, Color.Cyan, 1);
                }
                else
                {
                    CircleRender.Draw(Player.Position, GetBonudRange() - 29, Color.Cyan, 1);
                }
            }
        ...
        private void LogicQ()
        {
            ...
            if (Farm && !FishBoneActive && !Player.IsWindingUp && orbT == null && Orbwalker.CanAttack() && farmQout.Enabled && Player.Mana > WMANA + EMANA + RMANA + 20)
            {
                ...
                {
                    var hpPred = EnsoulSharp.SDK.HealthPrediction.GetPrediction(minion, 400);
        ...
        {
            return Player.PreviousPosition.Distance(EnsoulSharp.SDK.Prediction.GetPrediction(target, 0.05f).CastPosition) + Player.BoundingRadius + target.BoundingRadius;
        }
