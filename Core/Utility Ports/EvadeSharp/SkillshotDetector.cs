...
namespace Evade
{
    internal static class SkillshotDetector
    {
        ...
        private static AIHeroClient Jhin = null;
        private static Vector3 JhinLastRDirection;
        private static int JhinLastTimeR = 0;
        private static AIHeroClient MissFortune = null;
        static SkillshotDetector()
        {
            foreach (var hero in ObjectManager.Get<AIHeroClient>())
            {
                ...
                {
                    if (hero.CharacterName == "Jhin")
                        Jhin = hero;
                    if (hero.CharacterName == "MissFortune")
                        MissFortune = hero;
                }
            }
        ...
        private static void Obj_AI_Base_OnBuffAdd(AIBaseClient sender, AIBaseClientBuffAddEventArgs args)
        {
        }
        ...
        private static void Obj_AI_Base_OnNewPath(AIBaseClient sender, AIBaseClientNewPathEventArgs args)
        {
            ...
            if (args.IsDash && args.Path.Length >= 2)
            {
                ...
                var spellData = SpellDatabase.GetByDash(caster.CharacterName);
                ...
                {                    
                    DelayAction.Add(spellData.DashDelayedAction, () =>
                    {
                        if (spellData.CanDetectDash(sender, args))
                        {
                            TriggerOnDetectSkillshot(DetectionType.ProcessSpell, spellData, Utils.TickCount - Game.Ping / 2, startPos, endPos, sender.Position.To2D(), caster);
        ...
        private static void GameObject_OnCreate(GameObject sender, EventArgs args)
        {
            ...
            if (spellData != null)
            {
                if (Config.skillShots?[spellData.MenuItemName]?["Enabled" + spellData.MenuItemName] == null)
                    return;
                var caster = HeroManager.AllHeroes.Where(x => (x.IsEnemy || Config.TestOnAllies) && x.CharacterName == spellData.ChampionName)
                ...
                {
                    var clone = sender as AIMinionClient;
                ...
                {
                    var azirSoldier = ObjectManager.Get<AIMinionClient>().Where(x => (x.IsEnemy || Config.TestOnAllies) && x.Name == "AzirSoldier" && x.Health > 0).FirstOrDefault();
                ...
                EffectEmitter objpartice = sender as EffectEmitter;

                if (objpartice != null && spellData.Type != SkillShotType.SkillshotCircle)
                {
                    direction = objpartice.Position.To2D();
                ...
                if (spellData.SpellName == "XerathArcanopulse")
                {
                    if (!Config.TestOnAllies)
                        foreach (var ally in HeroManager.Allies.Where(x => (x.CharacterName == spellData.ChampionName)))
        ...
        private static void ObjAiHeroOnOnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            {
                Game.Print(Utils.TickCount + " ProcessSpellCast: " + args.SData.Name);
            ...
            if (spellData.SpellName == "YorickE")
            {
                ...
                startTime += (int)(sender.Position.Distance(LeagueSharpCommon.Geometry.Geometry.To3D(startPos)) / 1.8f);
            }
            ...
            {
                spellData.Delay = (int)(args.CastTime * 1000);
            }
            ...
            if (spellData.SpellName == "ThreshQInternal")
            {
                ...
                foreach (var point in points)
                {
                    ...
                    {
                        TriggerOnDetectSkillshot(DetectionType.ProcessSpell, spellData, startTime, startPos, point, point, (AIHeroClient) sender);
                    }
                }
                return;
            }
            if (spellData.SpellName == "MissFortuneRicochetShot" && args.Target != null && !args.Target.IsMe)
            {
                ...
                TriggerOnDetectSkillshot(DetectionType.ProcessSpell, spellData, startTime, startPos, endPos, endPos, (AIHeroClient) sender);
                return;
            }
            if(spellData.SpellName == "EkkoR")
            {
                var obj = ObjectManager.Get<AIMinionClient>().FirstOrDefault(x => x.Name.Contains("Ekko"));
               if(obj != null)
