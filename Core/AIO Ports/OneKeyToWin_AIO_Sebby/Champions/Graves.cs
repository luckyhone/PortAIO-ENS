    ...
    class Graves : Base
    {
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            if (qRange.Enabled)
            {
                if (onlyRdy.Enabled)
                {
                    if (Q.IsReady())
                    {
                        CircleRender.Draw(Player.Position, Q.Range, Color.Cyan.ToSharpDxColor(), 1);
                    }
                }
                else
                {
        ...
        private void LogicQ()
        {
            ...
            if (t.IsValidTarget())
            {
                ...
                else if (Harass && harassQ.Enabled && Player.Mana > QMANA + QMANA + WMANA + EMANA + RMANA && InHarassList(t))
                {
                    CastSpell(Q, t);
                }
                else
            ...
            else if (FarmSpells && farmQ.Enabled)
            {
                ...
                if (farmRsQ.MinionsHit >= LCminions.Value)
                {
