    ...
    public class Ward
    {
        ...
        public static InventorySlot GetConWardSlot()
        {
            ...
            return (from wardId in wardIds
                    where Items.CanUseItem(ObjectManager.Player,wardId)
        ...
        public bool CheckRemove()
        {
            ...
            if (AliveTo == float.MaxValue &&
                ObjectManager.GetUnitByNetworkId<AIBaseClient>(NetworkId) != null &&
                ObjectManager.GetUnitByNetworkId<AIBaseClient>(NetworkId).IsDead)
            {
        ...
        public static void DrawWardSpots()
        {
            foreach (Vector3 wardPos in WardSpots)
            {
                ...
                if (IsOnScreen(screenPos[0], screenPos[1]) && ObjectManager.Player.Distance(wardPos) < PerfectWardReborn.PerfectWardTracker.Config["drawings"]["drawDistance"].GetValue<MenuSlider>().Value)
                {
                    CircleRender.Draw(wardPos, WARD_INDICATOR_RADIUS, wardColor.ToSharpDxColor());
        ...
        public static void DrawSafeWardSpots()
        {
            foreach (WardSpot safeWardSpot in SafeWardSpots)
            {
                ...
                if (IsOnScreen(screenPos[0], screenPos[1]) && ObjectManager.Player.Distance(safeWardSpot.MagneticPosition) < PerfectWardReborn.PerfectWardTracker.Config["drawings"]["drawDistance"].GetValue<MenuSlider>().Value)
                {
                  CircleRender.Draw(safeWardSpot.WardPosition, 31.0f, wardColor.ToSharpDxColor());
                  CircleRender.Draw(safeWardSpot.WardPosition, 32.0f, wardColor.ToSharpDxColor());

                  CircleRender.Draw(safeWardSpot.MagneticPosition, 99.0f, wardColor.ToSharpDxColor());
                  CircleRender.Draw(safeWardSpot.MagneticPosition, 100.0f, wardColor.ToSharpDxColor());
