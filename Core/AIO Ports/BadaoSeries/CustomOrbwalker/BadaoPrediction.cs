...
using PortAIO;
using SharpDX;
using HitChance = SebbyLib.HitChance;
    ...
    public static class BadaoPrediction
    {
        ...
        private static void AIHeroClient_OnProcessSpellCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            if (sender.IsValid && sender.Team != ObjectManager.Player.Team && args.SData.Name == "YasuoWMovingWall")
            {
                ...
                _yasuoWallCastedPos = sender.Position.To2D();
            }
        }
            ...
            public CollisionableObjects[] CollisionObjects =
        {
            ...
            public Vector3 From
            {
                get { return _from.To2D().IsValid() ? _from : ObjectManager.Player.Position; }
                set { _from = value; }
            }
            ...
            public Vector3 RangeCheckFrom
            {
                get
                {
                    return _rangeCheckFrom.To2D().IsValid()
                        ? _rangeCheckFrom
                        : (From.To2D().IsValid() ? From : ObjectManager.Player.Position);
                }
        ...
        public class PredictionOutput
        {
            ...
            internal PredictionInput Input = new PredictionInput();
            ...
            {
                get { return _unitPosition.To2D().IsValid() ? _unitPosition.SetZ() : Input.Unit.Position; }
                set { _unitPosition = value; }
            }
        }
        public static bool BadaoCast(this Spell spell, AIBaseClient target)
        {
            var prediction = spell.GetBadaoPrediction(target);
            if (!spell.IsSkillShot)
                return false;
            if (prediction.Hitchance < (HitChance) spell.MinHitChance)
                return false;
            return ObjectManager.Player.Spellbook.CastSpell(spell.Slot, (Vector3) prediction.CastPosition);
        }
        
        public static PredictionOutput GetBadaoPrediction(this Spell spell, AIBaseClient target, bool collideyasuowall = true)
        {
            ...
            if (target.IsDashing())
            {
                ...
                result = spell.GetBadaoStandarPrediction(target,
                    new List<Vector2>() {target.Position.To2D(), dashDtata.Path.Last()},dashDtata.Speed);
            ...
            if (result == null)
            {
                result = spell.GetBadaoStandarPrediction(target,target.Path.ToList().ToVector2());
            }
            //Check for collision
            if (spell.Collision)
            {
        ...
        public static PredictionOutput GetBadaoStandarPrediction(this Spell spell, AIBaseClient target,
            List<Vector2> path, float speed = -1)
        {
            // check the unit speed input
            speed = (Math.Abs(speed - (-1)) < float.Epsilon) ? target.MoveSpeed : speed;
            // set standar output
            Vector2 castpos = target.Position.To2D();
            Vector2 unitpos = target.Position.To2D();
            ...
            if (path.Count <= 1)
            {
                ...
                castpos = target.Position.To2D();
                unitpos = target.Position.To2D();
                // target in range
                ...
                {
                    // skill shot circle
                    if (spell.Type == (SpellType) SkillshotType.SkillshotCircle)
                    {
            ...
            if (Math.Abs(spell.Speed - float.MaxValue) > float.Epsilon)
            {
                ...
                if (castpos01.IsValid() && castpos02.IsValid() && castpos03.IsValid())
                {
                    ...
                    if (spell.Type == (SpellType) SkillshotType.SkillshotCircle ||
                        (spell.Type == (SpellType) SkillshotType.SkillshotLine && Acosb <= 110 && Acosb >= 70))
                    {
                        List<Vector2> poses = new List<Vector2>();
                        for (int i = 0; i <= 10; i++)
                        {
        ...
        public static List<AIBaseClient> GetCollision(this Spell spell, List<Vector3> positions)
        {
                ...
                foreach (var objectType in objects)
                {
                    switch (objectType)
                    {
                        case CollisionableObjects.Minions:
                            ...
                            {
                                var target = minion;
                                var minionPrediction = spell.GetBadaoStandarPrediction(target,target.Path.ToList().ToVector2());
                                result.Add(minion);
                            }
                            break;
                        case CollisionableObjects.Heroes:
                            foreach (var hero in
                                GameObjects.EnemyHeroes.Where(hero => hero.IsValidTarget(Math.Min(spell.Range + spell.Width + 100, 2000), true, spell.RangeCheckFrom))
                                )
                            {
                                var target = hero;
                                var prediction = spell.GetBadaoStandarPrediction(target, target.Path.ToList().ToVector2());
                                result.Add(hero);
                            }
                            break;

                        /*case CollisionableObjects.All:
                            foreach (var hero in
                                GameObjects.AllyHeroes.Where(
                                    hero =>
                                       Vector3.Distance(ObjectManager.Player.Position, hero.Position) <= Math.Min(spell.Range + spell.Width + 100, 2000))
                                )
                            {
                                var target = hero;
                                var prediction = spell.GetBadaoStandarPrediction(target, target.Path.ToList().ToVector2());
                                if (
                                    prediction.UnitPosition.To2D()
                                        .Distance(spell.From.To2D(), position.To2D()) <=
