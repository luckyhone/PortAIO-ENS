...
namespace Flowers_Yasuo.MyCommon
{
    public class MyEventManager : MyLogic
    {
            ...
            try
            {
                ...
                if (IsMyDashing)
                {
                    if (FleeMenu["FlowersYasuo.FleeMenu.EQ"].GetValue<MenuBool>().Enabled && Q.IsReady() && !HaveQ3)
                    {
                        ...
                        {
                            if (MiscMenu["FlowersYasuo.MiscMenu.exploit"].GetValue<MenuBool>().Enabled &&
                                Me.IsDashing() && Me.GetDashInfo().EndPos.DistanceToPlayer() > 5)
                            {
        ...
        private static void OnAction(AfterAttackEventArgs Args)
        {
            try
            {
        ...
        private static void OnRender()
        {
            try
            {
                if (DrawMenu["FlowersYasuo.DrawMenu.Q"].GetValue<MenuBool>().Enabled && 
                    Q.IsReady() && !HaveQ3)
                {
                    CircleRender.Draw(Me.Position, Q.Range, System.Drawing.Color.FromArgb(86, 0, 255).ToSharpDxColor(), 1);
                }

                if (DrawMenu["FlowersYasuo.DrawMenu.Q3"].GetValue<MenuBool>().Enabled && 
                    Q3.IsReady() && HaveQ3)
                {
                    CircleRender.Draw(Me.Position, Q3.Range, System.Drawing.Color.FromArgb(0, 255, 161).ToSharpDxColor(), 1);
                }

                if (DrawMenu["FlowersYasuo.DrawMenu.E"].GetValue<MenuBool>().Enabled && 
                    E.IsReady())
                {
                    CircleRender.Draw(Me.Position, E.Range, System.Drawing.Color.FromArgb(0, 136, 255).ToSharpDxColor(), 1);
                }

                if (DrawMenu["FlowersYasuo.DrawMenu.R"].GetValue<MenuBool>().Enabled && 
                    R.IsReady())
                {
                    CircleRender.Draw(Me.Position, R.Range, System.Drawing.Color.FromArgb(251, 0, 133).ToSharpDxColor(), 1);
                }
            }
            catch (Exception ex)
            {
