...
namespace Easy_Sup.scripts
{
    class Pyke
    {
        ...
        public static void On_Load()
        {
            ...
            Q.SetSkillshot(0.25f, 70f, 2000, true, SpellType.Line);
            Q.SetCharged("PykeQ", "PykeQ", 400, 1030, 1.0f);
            E = new Spell(SpellSlot.E, 550f);
            E.SetSkillshot(0.275f, 70f, 500f, false, SpellType.Line);
            R = new Spell(SpellSlot.R, 750f);
            R.SetSkillshot(0.25f, 100f, float.MaxValue, false, SpellType.Circle);
        ...
        {
            var al = GameObjects.EnemyHeroes.Where(x => !x.IsDead && x.IsEnemy && !x.IsInvulnerable && x.Health < R.GetDamage(x) && x.DistanceToPlayer() < R.Range);
        ...
        private static void Combo()
        {
            ...
            {
                var rt = TargetSelector.GetTarget(R.Range,DamageType.Physical);
                if (rt != null && rt.IsValidTarget(R.Range))
                {
                    if (Menubase.Pyke_Combat.Rkill.Enabled && rt.Health > R.GetDamage(rt))
                    {
                        return;
                    }
        ...
        private static void Clear()
        {
            ...
            if (Menubase.Pyke_Clear.Ec.Enabled && E.IsReady())
            {
