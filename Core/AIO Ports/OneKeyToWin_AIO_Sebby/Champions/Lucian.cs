    ...
    class Lucian : Base
    {
        ...
        private void Drawing_OnDraw(EventArgs args)
        {
            if (qRange.Enabled)
            {
                if (onlyRdy.Enabled)
                {
                    if (Q.IsReady())
                    {
                        CircleRender.Draw(Player.Position, Q1.Range, Color.Cyan.ToSharpDxColor(), 1);
                    }
                }
                else
                {
            ...
            if (wRange.Enabled)
            {
                if (onlyRdy.Enabled)
                {
                    if (W.IsReady())
                    {
                        CircleRender.Draw(Player.Position, W.Range, Color.Orange.ToSharpDxColor(), 1);
                    }
                }
                else
                {
            ...
            if (eRange.Enabled)
            {
                if (onlyRdy.Enabled)
                {
                    if (E.IsReady())
                    {
                        CircleRender.Draw(Player.Position, E.Range, Color.Yellow.ToSharpDxColor(), 1);
                    }
                }
                else
                {
            ...
            if (rRange.Enabled)
            {
                if (onlyRdy.Enabled)
                {
                    if (R.IsReady())
                    {
                        CircleRender.Draw(Player.Position, R.Range, Color.Gray.ToSharpDxColor(), 1);
                    }
                }
                else
                {
        ...
        private void LogicQ()
        {
            ...
            if (t.IsValidTarget())
            {
                ...
                else if (Harass && InHarassList(t) && Player.Mana > QMANA + WMANA + EMANA + RMANA)
                {
                    Q.Cast(t);
                }
            }
            else if ((Combo || Harass) && harassQ.Enabled && t1.IsValidTarget() && InHarassList(t1) && Player.Distance(t1.PreviousPosition) > Q.Range + 100)
            {
                ...
                if (prepos.Hitchance < SebbyLib.HitChance.VeryHigh)
                {
                    return;
                }
        ...
        private void LogicW()
        {
            ...
            if (t.IsValidTarget())
            {
                ...
                else if (Harass && InHarassList(t) && !Player.IsUnderEnemyTurret() && Player.ManaPercent > 80 && Player.Mana > QMANA + WMANA + WMANA + EMANA + RMANA)
                {
                    CastSpell(W, t);
                }
