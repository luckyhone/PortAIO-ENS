 ...
 using SkillshotType = SebbyLib.SkillshotType;
 ...
 namespace Flowers_Viktor
{
    ...
    class Program
    {
        ...
        {
            if (GameObjects.Player.CharacterName != "Viktor")
                return;
        ...
        {
            if (GapcloserW.Enabled && W.IsReady())
            {
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (Menu["Flee"]["Key"].GetValue<MenuKeyBind>().Active)
            {
                ...
                if (Q.IsReady() && Menu["Flee"]["Q"].GetValue<MenuBool>().Enabled)
                {
        ...
        private static void KillSteal()
        {
            foreach (var e in GameObjects.EnemyHeroes.Where(e => e.IsValidTarget(1200) && !e.IsDead && !e.IsZombie()))
            {
                if(e != null)
                {
                    if(E.IsReady() && KSE.Enabled && e.Health < E.GetDamage(e))
                    {
                        CastE(e);
                    }
        ...
        private static void JungleClear()
        {
            if (Me.ManaPercent > JungleClearMana.Value)
            {
                ...
                if (MOB != null)
                {
                    ...
                    {
                        if (E.IsReady() && JungleClearE.Enabled)
                        {
                        ...
                        if (Q.IsReady() && JungleClearQ.Enabled)
                        {
                            Q.Cast(min);
                            return;
                        }
        ...
        private static void LaneClear()
        {
            if(Me.ManaPercent > LaneClearMana.Value)
            {
                ...
                if (mins != null)
                {
                    ...
                    {
                        if (E.IsReady() && LaneClearE.Enabled)
                        {
                            var EFarm = E.GetLineFarmLocation(mins, E.Width);

                            if (EFarm.MinionsHit >= LaneClearEMin.Value)
                            {
        ...
        {
            if (Me.ManaPercent > HarassMana.Value)
            {
        ...
        private static void Combo()
        {
            ...
            if (target != null && target.IsHPBarRendered)
            {
                ...
                if (Ignite.IsReady() && ComboIgnite.Enabled)
                    CastIgnite(target);
            }
        }
        ...
        {
            if (target != null && target.IsHPBarRendered && ComboQ.Enabled && Q.IsReady() && target.IsValidTarget(Q.Range))
            {
                Q.Cast(target);
                return true;
            }
        ...
        private static bool CastE(AIHeroClient target)
        {
            if (target != null && target.IsHPBarRendered)
            {
                var EInput = new PredictionInput()
                {
                    ...
                    Range = E.Range,
                    Speed = E.Speed,
                    Type = (Movement.SkillshotType) E.Type,
                ...
                if (target.IsValidTarget(E.Range))
                {
                    ...
                    if (MovePred.Hitchance >= (Movement.HitChance) HitChance.VeryHigh && FromPos != Vector3.Zero && ToPos != Vector3.Zero)
                    {
        ...
        private static void CastR(AIHeroClient e)
        {
            if (e != null && !e.IsZombie() && R.IsReady() && !Me.HasBuff("ViktorChaosStormTimer"))
            {
                ...
                if (e.HealthPercent > 5)
                {
                    ...
                    else if (!ComboROnlyKill.Enabled && e.IsValidTarget(600))
                    {
                        R.Cast(e.ServerPosition);
                    }
                }
