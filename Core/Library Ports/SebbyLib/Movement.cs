...
namespace Movement
{
    ...
    public static class Prediction
    {
        ...
        internal static HitChance GetHitChance(PredictionInput input)
        {
            ...
            if (hero.IsCastingImporantSpell() || hero.IsRecalling())
            {
                return HitChance.VeryHigh;
            }
        ...
        internal static PredictionOutput GetPositionOnPath(PredictionInput input, List<Vector2> path, float speed = -1)
        {
                ...
                Math.Abs(input.Speed - float.MaxValue) > float.Epsilon)
            {
                ...
                for (var i = 0; i < path.Count - 1; i++)
                {
                    ...
                    var sol = SebbyLib.Geometry.VectorMovementCollision(a, b, speed, input.From.ToVector2(), input.Speed, tT);
                    var t = (float)sol[0];
    ...
    internal static class AoePrediction
    {
        ...
        public static class Line
        {
            ...
            internal static Vector2[] GetCandidates(Vector2 from, Vector2 to, float radius, float range)
            {
                ...
                var intersections = PortAIO.Geometry.CircleCircleIntersection(
    ...
    public static class Collision
    {
        ...
        private static bool MinionIsDead(PredictionInput input, AIBaseClient minion, float distance)
        {
            ...
            if (SebbyLib.HealthPrediction.LaneClearHealthPrediction(minion, convert, 0) <= 0)
            {
                return true;
            }
            else
